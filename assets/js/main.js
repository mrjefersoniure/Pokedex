...
function openModal(pokemon) {
    ...
    modalContent.innerHTML = `
        <section class="pokemon_detail">
            ...
            <h3>Base stats</h3>
              <ol class="stats">
                    ${pokemon.types.map((pokemon) => `<li class="type">${pokemon.type.name}</li>`).join('')}
                </ol>
            <ol class="stats">
            ${pokemon.stats.map((pokemon) => `
                <li class="type">${pokemon.stat.name} 
                <span class="value"><meter value="${pokemon.base_stat}" min="0" max="100">${pokemon.base_stat}%</meter> ${pokemon.base_stat}</span>
                </li>`
            ).join('')}
            </ol>
        </section>
